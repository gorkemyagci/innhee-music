...
const FAQ = () => {
    ...
    const faqItems: FAQItem[] = Object.entries(t.raw("items")).map(([key, item]) => ({
        title: (item as any).title,
        description: (item as any).description
    }));

    return (
        <div className="relative max-w-[1200px] mx-auto pt-8 md:pt-12 lg:pt-20 pb-10 md:pb-12 lg:pb-20 w-full">
            <div className={cn(
                ...
                <div className="flex flex-1 flex-col items-start gap-4 md:gap-5">
                    <span className="text-black font-semibold text-3xl md:text-4xl lg:text-5xl">{t("title")}</span>
                    <span className="text-base md:text-lg font-normal text-[#666D80]">{t("subtitle")}</span>
                    <Button
                        type="button"
                        ...
                        {t("viewAll")} <Icons.arrow_right className="size-3 md:size-4" />
                    </Button>
                </div>
