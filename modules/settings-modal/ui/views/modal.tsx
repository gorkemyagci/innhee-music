...
const Modal = ({ children, open = false, setOpen = () => { } }: ModalProps) => {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen} modal={true}>
            ...
            <DialogContent className="sm:max-w-5xl p-0 h-[680px] overflow-hidden gap-0 z-50 w-full flex items-start justify-start bg-white rounded-3xl">
                <div className="w-56 flex flex-col items-start gap-2.5 p-6 border-r border-soft-200 h-full">
                    <DialogHeader>
                        <DialogTitle className="text-soft-400 font-medium text-xs">SETTINGS</DialogTitle>
                    </DialogHeader>
                    <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} />
                </div>
                <div className="flex-1 py-4 custom-scroll overflow-y-auto h-full">
